...
module.exports = router => {
  ...
  router.post('/doi/:id/pass', checkPermission(IS_USER), async (req, res, next) => {
    try {
      ...
      team.submissions[0].isPass = !team.submissions[0].isPass;
      let x = await mongoose.model('teams').findOneAndUpdate({ _id: req.params.id }, team, { new: true });
      let findTeam = await mongoose.model('randCode').findOne({ teamId: team._id });
      if (!findTeam && team.submissions[0].isPass == true) {
        await mongoose.model('randCode').create(rand);
      }
      if(findTeam && team.submissions[0].isPass== false) {
        await mongoose.model('randCode').deleteOne(rand);
      }
      return redirect('../../doi');
    }
    catch (err) {
